package com.nrpt.springcloud.search.service.impl;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.TypeReference;
import com.nrpt.springcloud.common.to.es.SkuEsModel;
import com.nrpt.springcloud.common.utils.R;
import com.nrpt.springcloud.search.config.nrptElasticSearchConfig;
import com.nrpt.springcloud.search.constant.EsConstant;
import com.nrpt.springcloud.search.feign.ProductFeignService;
import com.nrpt.springcloud.search.service.MallSearchService;
import com.nrpt.springcloud.search.vo.AttrResponseVo;
import com.nrpt.springcloud.search.vo.BrandVo;
...
public class MallSearchServiceImpl implements MallSearchService {
    ...
    public SearchResult search(SearchParamVo paramVo) {
        ...
        try {
            //执行检索请求
            SearchResponse response = client.search(searchRequest, nrptElasticSearchConfig.COMMON_OPTIONS);
...
//        result.setPageNum();
        ...
        if (paramVo.getAttrs() != null && paramVo.getAttrs().size() > 0) {
            List<SearchResult.NavVo> collect = paramVo.getAttrs().stream().map(attr -> {
                ...
                navVo.setLink("http://search.nrpt.com/list.html?" + replace);
                return navVo;
            }).collect(Collectors.toList());
